...
prefix = "voice_data_gpt"
_prefix = len(prefix)+1

data_path = "data\\data_gpt.json"
...
response = "Извините, кажется, я не могу найти нужную информацию в моей базе данных. Возможно, моя база не располагает этими данными или это слишком специфический запрос. Пожалуйста, попробуйте переформулировать вопрос или задать его иначе, чтобы я могла точнее помочь вам."
...
async def main():
    ...
    file, id = await synthesis(response, prefix=prefix)
    ...
    results = {
        "text": response, "id": str(id),
        "file_path": file_name, "file_content": file_content[0]}
